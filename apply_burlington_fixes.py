...
file_path = "/data/msp-site/ai-governance-burlington.html"

changes = [
    (
        "AI Governance for Burlington",
        "AI Governance & Automation for Professional Services Businesses in Burlington"
    ),
    (
        "Trueline IT gives your Burlington business a documented AI policy, safe tools, and verifiable compliance — for a flat monthly fee.",
        "Trueline IT gives your Burlington business a documented AI policy, safe tools, and ongoing compliance management — for a flat monthly fee."
    ),
    (
    ...
    (
        # We handle "For Burlington" with an exact match or close match due to potential HTML tags around it,
        # but let's try the literal first
        '>For Burlington<',
...
for i, (old_text, new_text) in enumerate(changes, 1):
    if old_text in content:
        if i == 3: # the `>For Burlington<`
            content = content.replace(old_text, new_text, 1) # Only do it once or all? Let's just trust it.
        content = content.replace(old_text, new_text)
        print(f"Change {i}: SUCCESS")
    else:
        # Fallback for exact literal match without angle brackets:
        if i == 3 and "For Burlington" in content:
             content = content.replace("For Burlington", "The Trueline IT Service", 1)
